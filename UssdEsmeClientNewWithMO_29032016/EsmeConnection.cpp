	...
	switch(CL_begin.mcu8_ussdOptype){
		case USSD_OPTYPE::USSD_REQUEST:
			//TODO::this will come in case of PSSR
			{
				...
				pcL_Msg->pmcC_EsmeMsg->set_srv_code((char*)CL_begin.pmcu8_serviceCode);
				...
				pcL_Msg->meC_traversolPath.assign(pcL_Msg->pmcC_EsmeMsg->srv_code().c_str());
                                ...
                                CL_menuDetails.mcC_menuServiceCode.assign((char*)CL_begin.pmcu8_serviceCode); 
				...
				if(CL_msg.empty()){
					DBG_VERBOSE((CG_EventLog),("Menu Not cofigure for this message:%s[%s]",CL_begin.pmcu8_serviceCode,"MO"));
					delete pcL_Msg;
					return false;
				}
