...
bool CDBPicker::mefn_updateScheduleMappingStaus(){
	...
	std::stringstream CL_QueryStream;
	CL_QueryStream.str("");
	CL_QueryStream<<"update schedule_table_mapping set STATUS ='P' where STATUS='E' and SCHEDULE_ID in (select SCHEDULE_ID from schedule_profile where STATUS in (2,5) AND CHANNEL_ID in ("<<CG_Cfg.mcfn_getChannelId()<<"))";
	if(CL_dbObj.mcfn_ExecuteQuery(CL_QueryStream.str())!=0)
		return false;
	return true;
}
