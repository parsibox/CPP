...
/*			memcpy(&mcsi_cmdLength,&u8L_header[0],4);
			...
			memcpy(&mcsi_sessionId,&u8L_header[12],4);
			mcsi_cmdLength = htonl(mcsi_cmdLength);
			mcsi_cmdId = htonl(mcsi_cmdId);
			mcsi_cmdStatus = htonl(mcsi_cmdStatus);
			mcsi_sessionId = htonl(mcsi_sessionId);*/
