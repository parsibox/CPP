...
bool CEsmeClient::mcfn_sendXmlMsg(std::string CL_xmlMsg,long long iL_seqNum[2],int iL_operation)
{
...
bool CEsmeClient::mcfn_sendUssdBind()
{
	...
	printf("%s\n",CL_bind.mcfn_encode().c_str());
        long long lL_seqId[2];
        lL_seqId[0]=mesi_reqId;
        lL_seqId[1]=0;
	return mcfn_sendXmlMsg(CL_bind.mcfn_encode(),lL_seqId,XML_LOGIN);
}

bool CEsmeClient::mcfn_sendUssdBindResp(long long sL_seqId,int iL_status,std::string CL_authMsg,int iL_reqId)
{
	...
	return mcfn_sendXmlMsg(CL_pdu.mcfn_encode(),lL_seqId,XML_LOGIN_RESP);
}
...
{
	long long sL_seqId[2];
	sL_seqId[0]=mcfn_getReqNumber();
	sL_seqId[1]=0;
	return mcfn_sendXmlMsg(CL_pdu.mcfn_encode(),sL_seqId,XML_START);
}
bool CEsmeClient::mcfn_sendUssdContinue(CXmlUssdMsg &CL_pdu,long long sL_seqId[2])
{
	return mcfn_sendXmlMsg(CL_pdu.mcfn_encode(),sL_seqId,XML_CONTINUE);
}

bool CEsmeClient::mcfn_sendUssdEnd(CXmlUssdMsg &CL_pdu,long long sL_seqId[2])
{
