...
class CEsmeClient{
	...
	public:
		...
		bool mcfn_sendXmlMsg(std::string CL_xmlMsg,long long iL_seqNum[2],int iL_operation);
		//clinet  functions	
		bool mcfn_sendUssdBind();
		bool mcfn_sendUssdShake();
		//common functions	
		bool mcfn_sendUssdBegin(CXmlUssdMsg &);
		bool mcfn_sendUssdContinue(CXmlUssdMsg &,long long sL_seqId[2]) ;
		bool mcfn_sendUssdEnd(CXmlUssdMsg &,long long sL_seqId[2]);
		...
		bool mcfn_sendUssdBindResp(long long iL_seqId,int iL_status,std::string CL_accName,int iL_reqId);
