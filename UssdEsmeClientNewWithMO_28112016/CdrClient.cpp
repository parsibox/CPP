...
bool CCdrClient::mcfn_run(int iL_threadId,void *vL_arg){
	while(1){
		...
		if(pCL_Msg->submit_msg().service_url().empty()==false){
			...
			CL_req<<pCL_Msg->submit_msg().service_url()
                              <<"MSISDN="<<pCL_Msg->submit_msg().da()
                              <<"&UNIQUE_ID="<<pCL_Msg->submit_msg().msg_id()
                              <<"&STATUS="<<pCL_Msg->submit_msg().status()
                              <<"&TRAVERSOL_PATH"<<pCL_Msg->submit_msg().msg();
