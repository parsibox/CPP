...
CEsmeClient::CEsmeClient(int iL_hdrLen,CSTSSocket* sock):CTcpClient("SMPP",10){
        meb_isObjectDeleted = false;
        mesi_headerLen = iL_hdrLen;
        pmeC_serverSock = sock;
	mcfn_start();
       meb_isThreadRunning = true;
       mesi_shakeCount=0;
}

CEsmeClient::~CEsmeClient()
{
 meb_isObjectDeleted = true; 
 while(meb_isThreadRunning)
 sleep(1);
}
...
bool CEsmeClient::mcfn_run(int,void *){
	...
	if(!meC_RecDataQue.mcfb_getNextMessage(CL_recData,false)){	
		sleep(1);
                mesi_shakeCount++;
                if(meb_isObjectDeleted)
                 {
                     meb_isThreadRunning=false;
                     return false;
                 }
               if(mesi_shakeCount == 30)
               {
                     mesi_shakeCount=0;
                     mcfn_sendUssdShake(); 
               }
...
bool CEsmeClient::mcfn_sendUssdMessage(UssdData &CL_pdu){
	...
	{
                CL_pdu.set_receiver_session_id(mcfn_getSeqNumber());
		DBG_INFO((CG_EventLog),("receive session id assigned [%ld]",CL_pdu.receiver_session_id()));
