...
std::string  CMenuTree::mcfn_getMenuList(void* pSL_ActiveNode, char* pscL_Buffer, bool* pbL_IsLeaf)
{
	...
	{
		SL_DataNode.NodeNature	= pSL_ActiveMenuNode->nodeNature;
		strcpy(SL_DataNode.Keyword, pSL_ActiveMenuNode->keyword);
		//strcpy(SL_DataNode.Msg, pSL_ActiveMenuNode->message);
	     	
		strcpy(SL_DataNode.Description, pSL_ActiveMenuNode->description);
		strcpy(SL_DataNode.DestinationAddr, pSL_ActiveMenuNode->destinationAddress);
		memcpy(pscL_Buffer, &SL_DataNode, sizeof(SL_DataNode));
		*pbL_IsLeaf	=	true;
