...
void MsgTypes::mcfn_onTimerExpiry(void *p){
		...
		CHeader CL_header;
		CUssdMsg CL_pdu;
		CL_pdu.mcC_requestId=mcC_reqId;
		...
		try{
			...
			if(!CG_SmppQue.mcfb_getNextMessage(pcL_Msg,false)){
				...
				if(pcL_Msg->pmcC_EsmeMsg->msg_type()==2){
					...
					CL_pdu.mcC_msgType=mcC_SmppClient.mcfn_convertToString(USSR);
				}
				else{//USSN
					CL_msg=pcL_Msg->pmcC_EsmeMsg->msg().c_str();
					CL_pdu.mcC_msgType=mcC_SmppClient.mcfn_convertToString(USSN);
					pcL_Msg->pmcC_EsmeMsg->set_srv_code(pcL_Msg->pmcC_EsmeMsg->oa());
				}
				CL_pdu.mcC_requestId= mcC_SmppClient.mcfn_convertToString(mcC_SmppClient.mcfn_getReqNumber());
				DBG_VERBOSE((CG_EventLog),("Menu Sending to USSD:%s",CL_msg.c_str()));	
				pcL_Msg->pmcC_EsmeMsg->set_submit_time(time(NULL));
				pcL_Msg->pmcC_EsmeMsg->set_submit_msgid(atoi(CL_pdu.mcC_requestId.c_str()));
				pcL_Msg->pmcC_EsmeMsg->set_sender_sessionid(atoi(CL_pdu.mcC_requestId.c_str()));
				...
				if(mcC_SmppClient.mcfn_sendUssdBegin(CL_pdu)==false){	
					...
					pcL_Msg->mcC_reqId=CL_pdu.mcC_requestId;
					CG_seqMap.mcfb_insert(CL_pdu.mcC_requestId,pcL_Msg);
					...
					DBG_VERBOSE((CG_EventLog),("Message Sent to SMSC:%s",CL_pdu.mcC_requestId.c_str()));
					++iL_MsgSubmitCounter;
