		...
		case  USSD_OPTYPE::USSD_RESPONCE:
			...
			if(CG_seqMap.mcfb_findElement(CL_header.mcsi_senderCB,pcL_Msg)){
				if(pcL_Msg->pmcC_EsmeMsg->msg_type() == 2 ){
					...
					CL_msg=CMenu::mcfnS_getLeafData(pcL_Msg->pmcC_Session,(const char*)CL_continue.pmcu8_ussdString,pcL_Msg->pmcC_EsmeMsg->username().c_str(),pcL_Msg->pmcC_EsmeMsg->srv_code().c_str(),bL_isLeaf,CL_url);
				}
