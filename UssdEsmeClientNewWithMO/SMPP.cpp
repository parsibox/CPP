...
void MsgTypes::mcfn_onTimerExpiry(void *p){
	...
	CHeader CL_header;
	u8 pu8L_data[208]={0x00};
	CL_header.mcsi_receiverCB=mcsi_seqNum;
	CL_header.mcsi_cmdLength=COMMAND_ID::USSD_END_LEN;
	CL_header.mcsi_cmdId=COMMAND_ID::USSD_END;
	CL_header.mcsi_cmdStatus=1034;
	CUssdEnd CL_end(CL_header);
	CL_end.mcu8_ussdOptype=USSD_OPTYPE::USSD_RELEASE;
	pCG_Smpp->mcC_SmppClient.mcfn_onUssdEnd(CL_header,CL_end.mcfn_getParam(pu8L_data),207);
	...
	while(1){
		try{
			...
			if(!CG_SmppQue.mcfb_getNextMessage(pcL_Msg,false)){
				...
				{
					if(!mcC_SmppClient.mcfn_sendUssdShake())
                                         {
				...
				if(pcL_Msg->pmcC_EsmeMsg->msg_type()==2){
                                        std::string CL_username = pcL_Msg->pmcC_EsmeMsg->username().c_str();
                                        std::string CL_srvCode = pcL_Msg->pmcC_EsmeMsg->srv_code().c_str();
					CL_msg=CMenu::mcfnS_getRootMenuData(CL_username,CL_srvCode,pcL_Msg->pmcC_EsmeMsg->msg().c_str(),pcL_Msg->pmcC_Session);
					if(CL_msg.empty()){
					...
					u8L_ussdOptype=USSD_OPTYPE::USSD_REQUEST;
				}
				else{//USSN
					CL_msg=pcL_Msg->pmcC_EsmeMsg->msg().c_str();
					u8L_ussdOptype=USSD_OPTYPE::USSD_NOTIFY;
					pcL_Msg->pmcC_EsmeMsg->set_srv_code(pcL_Msg->pmcC_EsmeMsg->oa());
				}
				CL_header.mcsi_senderCB=mcC_SmppClient.mcfn_getSeqNumber();
				...
				if(mcC_SmppClient.mcfn_sendUssdBegin(CL_header,pcL_Msg->pmcC_EsmeMsg->da().c_str(),CG_Cfg.mcfn_getOpeartorServiceCode(),u8L_dcs,CL_msg.c_str(),u8L_ussdOptype)==false){	
					...
					pcL_Msg->mcsi_seqNum=CL_header.mcsi_senderCB;
					...
					DBG_VERBOSE((CG_EventLog),("Message Sent to SMSC:%d",CL_header.mcsi_senderCB));
					++iL_MsgSubmitCounter;
