...
*/
			  mcfn_sendUssdEnd(pcL_Msg->pmcC_EsmeMsg->da().c_str(),pcL_Msg->pmcC_EsmeMsg->srv_code(),(u8)pcL_Msg->pmcC_EsmeMsg->dcs(),CL_msg,USSD_OPTYPE::USSD_RELEASE);
		...
		/*	if(pcL_Msg->meb_isCdrGenerated || pcL_Msg->pmcC_EsmeMsg->msg_type() == 1 )
			...
			mcfn_sendUssdContinue(pcL_Msg->pmcC_EsmeMsg->da().c_str(),pcL_Msg->pmcC_EsmeMsg->srv_code(),0x00,CL_msg, USSD_OPTYPE::USSD_REQUEST ) ;
			CTimerManeger::mcfnS_GetInstance()->mcfn_registerTimer((ITimer*)pcL_Msg,CG_Cfg.mcfn_getUssdTimeout(),NULL);
		}
	}
	else{
