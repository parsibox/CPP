#ifndef _OWN_XML_USSD_SUBMIT_H_
#define _OWN_XML_USSD_SUBMIT_H_
#include"XmlUssdHeader.hpp"

class CXmlUssdMsg:public CXmlHeader{
	public:
		...
		CXmlUssdMsg(CXmlHeader &CL_hdr):CXmlHeader(CL_hdr)
		{
			mcfn_init();
		}
		...
		std::string  mcfn_encode(){
			...
			CL_xmlMsg<<"</" <<CL_parent <<">";
                        mcsi_cmdLength = CL_xmlMsg.str().length() + USSD_OWN_HEADERLEN;
