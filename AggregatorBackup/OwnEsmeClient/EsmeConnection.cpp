...
bool CEsmeConnection::mcfn_onUssdBegin(CXmlHeader& CL_header,CXmlUssdMsg &CL_pdu){
	...
	if(CL_msg.empty()){
        ...
        CG_seqMap.mcfb_insert( CL_header.mcsi_sessionId[1],pcL_Msg);
	mcfn_sendUssdContinue(CL_pdu,CL_header.mcsi_sessionId[1]);
	return true;
}
bool CEsmeConnection::mcfn_onUsddContinue(CXmlHeader& CL_header,CXmlUssdMsg &CL_pdu){
	...
	if(CG_seqMap.mcfb_findElement(CL_header.mcsi_sessionId[0],pcL_Msg)){
		...
		if(CL_msg.empty()||bL_isLeaf){
			...
			mcfn_sendUssdEnd(CL_pdu,CL_header.mcsi_sessionId[0]);
			...
			CL_pdu.mcC_userData=CL_msg;	
                        CL_pdu.mcC_msgType = "3";//mefn_getString(RESPONSE); 
			mcfn_sendUssdContinue(CL_pdu,CL_header.mcsi_sessionId[0]);
