...
bool CSTSSocket::mcfn_create(int slL_Port, bool bL_IsKeepAlive)
{
...
bool CSTSSocket::mcfn_create(int slL_Port,char * pscL_IP, bool bL_IsKeepAlive)
{
	mesl_ServerPort = slL_Port;
	meS_SockAddr.sin_family = AF_INET;
	meS_SockAddr.sin_port= htons(mesl_ServerPort);
...
bool CSTSSocket::mcfn_receive(char *psl_Buffer,int slL_TotLen,bool bL_block)
{
	int slL_NoByte = 0;
	int slL_Len = 0;
	...
	/*
	fd_set SL_SReady;
	struct timeval SL_NOWait; 
	memset(&SL_NOWait,0x00,sizeof(SL_NOWait));	
	SL_NOWait.tv_usec = 100;
	SL_NOWait.tv_sec = 0;
	*/
	/*using select end */
	...
	while(1)
	{
		...
		FD_ZERO(&SL_SReady);
		//add to master set
		FD_SET(meS_Socket, &SL_SReady);
		siL_ReturnValue = select(meS_Socket+1,&SL_SReady,NULL,NULL,&SL_NOWait);
		*/
		/* using select end */
		...
		{
			fds[1].fd = meS_fds[0];
			fds[1].events = POLLIN|POLLRDNORM|POLLRDBAND|POLLPRI|POLLERR|POLLHUP|POLLNVAL;
			fds[1].revents = POLLIN|POLLRDNORM|POLLRDBAND|POLLPRI|POLLERR|POLLHUP|POLLNVAL;
	
			siL_ReturnValue = poll(fds,2,timeout_msecs);
		}
		else
		*/
...
int CSTSSocket::mcfn_isDataAvailable(int siL_Timeout)
{
	int siL_ReturnValue = 0x00; 
	fd_set SL_SReady; 
	struct timeval SL_NOWait; 
	...
	FD_ZERO(&SL_SReady); 
	FD_SET((unsigned int)meS_Socket,&SL_SReady); 
	memset((char *)&SL_NOWait,0,sizeof(SL_NOWait)); 

	if(siL_Timeout == -1)
	{//wait infinetly
		siL_ReturnValue = select(meS_Socket+1,&SL_SReady,NULL,NULL,NULL); 
	}
	else
	{//wait till time given
		SL_NOWait.tv_usec = siL_Timeout;
		SL_NOWait.tv_sec = 0x00;
		//SL_NOWait.tv_sec = siL_Timeout;
		//SL_NOWait.tv_usec = 0x00;
		siL_ReturnValue = select(meS_Socket+1,&SL_SReady,NULL,NULL,&SL_NOWait);
	}
	/*
	   if(siL_ReturnValue == -1 || siL_ReturnValue == 0x00)
	   return siL_ReturnValue;

	   if(!FD_ISSET(meS_Socket,&SL_SReady)) 
	   return -1;
	 */
	return siL_ReturnValue;
}

int CSTSSocket::mbfn_getIpAddress(char *buf, size_t buflen)
{
...
int CSTSSocket::mbfn_getIPAndPort(char *buf, size_t buflen)
{
	char *ipAddress = inet_ntoa( meS_SockAddr.sin_addr );
...
#ifdef __LINUX__
bool CSTSSocket::mcfn_connect(char *pscL_IP, int siL_Timeout){
        bool bL_Return = true;
        meS_SockAddr.sin_addr.s_addr = inet_addr(pscL_IP);
