...
void MsgTypes::mcfn_onTimerExpiry(void *p){
	...
	pCG_Smpp->mcC_SmppClient.mcfn_onUssdEnd(&CL_pdu);
	...
	while(1){
		try{
			...
			if(!CG_SmppQue.mcfb_getNextMessage(pcL_Msg,false)){
				...
				if(mcC_SmppClient.mcfn_sendUssdBegin(iL_sessionId,pcL_Msg->pmcC_EsmeMsg->da().c_str(),CG_Cfg.mcfn_getOpeartorServiceCode(),u8L_dcs,CL_msg.c_str(),u8L_ussdOptype)==false){	
