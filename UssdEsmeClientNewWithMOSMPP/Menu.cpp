...
bool CMenu::mcfnS_validateUserInput(std::string CL_input,int inputType)
{
	switch(inputType)
	{
		case 4://only numbers
		case 3://only alphabets
		case 2://only alphabets and numarics
			for(int i=0;i<CL_input.length();i++)
			{
				if(4==inputType && !isdigit(CL_input[i]))
				{
					DBG_CRITICAL((CG_EventLog),("User Input is having non-digit data:%s[%d]",CL_input.c_str(),inputType));
					return false;
				}
				if(3 == inputType && !isalpha(CL_input[i])){
					DBG_CRITICAL((CG_EventLog),("User Input is having non-alpha data:%s[%d]",CL_input.c_str(),inputType));
					return false;
				}
				if(2 == inputType && !isalnum(CL_input[i]))
				{ 
					DBG_CRITICAL((CG_EventLog),("User Input is having special chars:%s[%d]",CL_input.c_str(),inputType));
					return false;
				}
			}
			break;

		case 1://all charectors
		default:
			return true;
	}
	return true;
...
std::string CMenu::mcfnS_getLeafData(Session *&pCL_session,std::string CL_input,std::string CL_userName,std::string CL_serviceCode,bool &bL_IsLeaf,std::string &CL_url,bool &bL_isValidInput ){
	...
	{
		if(pCL_session->mcsi_activeMenuNodeType != 0 )
		{
			if(mcfnS_validateUserInput(CL_input,pCL_session->mcsi_activeMenuNodeType))
			{
			DBG_DEBUG((CG_EventLog),("user input is valid string"));
				pCL_session->mcC_userInputs.push_back(CL_input);	
				CL_input = "-1";
			}
			else
			{
		...
		if(bL_isValidInput && pCL_nodeMenu->mcfb_findElement(CL_input,pCL_nodeData) &&  pCL_nodeData->nextMenuId.compare("") == 0)
		{
			bL_IsLeaf = true;
	...
	if(!bL_isValidInput) CL_outStr<<"Invalid Input,";
	  ...
	  if(pCL_nodeData->isNodeMessageAtEnd)
        ...
        pCL_session->mcsi_activeMenuNodeType = pCL_nodeData->nodeType; 
	...
	if(CL_dbObj.mcfn_ExecuteQuery(CL_QueryStream.str(),CL_ResultSet)==0){
		if(CL_ResultSet.num_rows()>0){
			...
			for(int i=0;i<CL_ResultSet.num_rows();i++){
				...
				pCL_nodeData->nodeType=atoi(CL_ResultSet[i]["input_type"].c_str());
				pCL_nodeData->isNodeMessageAtEnd=((CL_ResultSet[i]["node_msg_at_end"].c_str()[0]=='Y')?true:false);
